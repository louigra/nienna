...
export async function renderAwardsTimeline(containerEl, { cip, agency, category, element }) {
  ...
  try {
    // Load all project dimensions, filter by ACEP fields in JS
    ...
    if (cip)      projects = projects.filter(p => String(p.cip      ?? '') === String(cip));
    ...
    // Load all estimate dimensions, pick latest per project (for dollar mode)
    ...
    if (projects.length) {
      ...
      const allEstimates = await Promise.all(
        estDimRows.map(async ({ id }) => {
          const fields = await db.getDimensionFields(id, ['project_id', 'total_amount', 'estimate_date']);
          return { id, ...fields };
        })
      );
